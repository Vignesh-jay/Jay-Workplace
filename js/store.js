...
function getAssetHistory() {
  return JSON.parse(localStorage.getItem(STORAGE_KEYS.assetHistory)) || [];
}

function saveAssets(assetList) {
  localStorage.setItem(STORAGE_KEYS.assets, JSON.stringify(assetList));
}
...
function saveAssetHistory(history) {
  localStorage.setItem(STORAGE_KEYS.assetHistory, JSON.stringify(history));
}
...
function getEmployeeHistory() {
  return JSON.parse(localStorage.getItem(STORAGE_KEYS.employeeHistory)) || [];
}

function saveEmployeeHistory(history) {
  localStorage.setItem(STORAGE_KEYS.employeeHistory, JSON.stringify(history));
}
...
function getAssignmentHistory() {
  return JSON.parse(localStorage.getItem('jay_assignment_history')) || [];
}

function saveAssignmentHistory(history) {
  localStorage.setItem('jay_assignment_history', JSON.stringify(history));
}
